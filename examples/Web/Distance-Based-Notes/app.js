...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const frequencyFactorValue = document.getElementById('frequency-factor-value');
  ...
  function updateNotesList() {
    ...
    if (notes.length === 0) {
      ...
      emptyState.textContent = 'No notes yet. Right-click to start a sequence and move to drop notes.';
  ...
  function updateDistanceDisplays() {
    ...
    currentDistanceDisplay.textContent = `${currentDistance.toFixed(2)} units (${currentMm} mm)`;
    totalDistanceDisplay.textContent = `${totalDistance.toFixed(2)} units (${totalMm} mm)`;
  ...
  function handleMouseMove(args) {
    ...
    if (isSequenceStarted) {
      ...
      if (Math.random() < 0.05) {
        console.log(`Delta values: dx=${dx}, dy=${dy}, distance=${movementDistance.toFixed(2)}`);
      }
        ...
        for (const note of notes) {
          ...
          if (distanceDifference <= replayTolerance) {
            ...
            if (!note.isPlaying) {
              ...
              if (Math.random() < 0.2) {
                const distanceInMm = convertToMillimeters(note.distance);
                addSystemMessage(`Replaying note at distance ${note.distance.toFixed(2)} units (${distanceInMm} mm)`);
              }
            }
          }
        }
  ...
  function handleRightDrag(args) {
    ...
    rightDragDistance = Math.sqrt(dragDeltaX * dragDeltaX + dragDeltaY * dragDeltaY);
  ...
  function handleLeftUp(args) {
    ...
    if (isLeftPressed && (Date.now() - leftPressStartTime) >= LONG_PRESS_THRESHOLD) {
  ...
  function dropNote() {
    ...
    const adjustedBaseFrequency = initialBaseFrequency * (1 + distanceAdjustment);
    ...
    // Log
    addSystemMessage(`Note dropped at distance ${totalDistance.toFixed(2)} units (${distanceInMm} mm) with frequency ${Math.round(frequency)} Hz (base: ${Math.round(adjustedBaseFrequency)} Hz)`);
  ...
  distanceThresholdSlider.addEventListener('input', () => {
    updateSliderValues();
    addSystemMessage(`Distance threshold set to ${distanceThresholdSlider.value} units`);
  });
  ...
  frequencyFactorSlider.addEventListener('input', () => {
    updateSliderValues();
    addSystemMessage(`Pitch increment factor set to ${frequencyDistanceFactor.toFixed(4)}`);
  });
