...
void inHIDMove(OSCMessage &msg) { // float values dx, dy (delta movement)
  int deviceID = msg.getInt(0);
  ...
  if (!gaugeMode) {
    ...
    if (abs(dx) > 5.0 || abs(dy) > 5.0) { // Only log for significant movements
      Serial.println("Gauge mode active - ignoring movement for colors");
    }
  }
}

void inHIDScroll(OSCMessage &msg) { // float values scrollx, scrolly (delta scroll)
  int deviceID = msg.getInt(0);
...
void updateStripColor(float dx, float dy) {
  ...
  if (movementMagnitude > 0.1) { // Reduced threshold from 0.5 to 0.1
...
void updateScrollGauge(float scrollValue) {
  ...
  // Keep within bounds
  if(scrollGaugeLevel < 0) scrollGaugeLevel = 0;
  if(scrollGaugeLevel > LED_COUNT) scrollGaugeLevel = LED_COUNT;
